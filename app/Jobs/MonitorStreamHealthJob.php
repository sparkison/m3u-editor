...
class MonitorStreamHealthJob implements ShouldQueue
{
    ...
    public function handle(HlsStreamService $hlsStreamService): void
    {
        ...
        if ($segments->isEmpty()) {
            ...
            if ($startTime && (time() - $startTime > $gracePeriod)) {
                ...
                Log::channel('ffmpeg')->debug(
                    "[Monitor][{$this->streamType} ID {$this->activeStreamId}, OrigReq ID {$this->originalModelId}] No .ts segments yet, but stream started recently (within grace period). Skipping failover."
                );
    ...
    {
        Log::channel('ffmpeg')->warning(
            "[SeqFail][OrigReq ID {$this->originalModelId}] Active stream {$this->streamType} ID {$this->activeStreamId} (index {$this->currentIndexInSourceIds}) failed health check. Initiating failover sequence."
        );
