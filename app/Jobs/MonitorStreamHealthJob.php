...
class MonitorStreamHealthJob implements ShouldQueue
{
    ...
    public function handle(HlsStreamService \$hlsStreamService): void
    {
        ...
        if (\$segments->isEmpty()) {
            ...
            if (\$startTime && (time() - \$startTime > \$gracePeriod)) {
                ...
                Log::channel('ffmpeg')->debug(
                    "[Monitor][{\$this->streamType} ID {\$this->activeStreamId}, OrigReq ID {\$this->originalModelId}] No .ts segments yet, but stream started recently (within grace period). Skipping age check."
                );
            ...
            if ((time() - \$latestSegmentTimestamp) > \$ageThresholdSeconds) {
                Log::channel('ffmpeg')->warning(
                    "[Monitor][{\$this->streamType} ID {\$this->activeStreamId}, OrigReq ID {\$this->originalModelId}] Latest segment in {\$hlsDirectoryPath} is too old (Timestamp: {\$latestSegmentTimestamp}, Current: " . time() . ", Threshold: {\$ageThresholdSeconds}s)."
                );
                \$this->handleStreamSequenceFailure(\$hlsStreamService);
                return;
            }
    ...
    {
        Log::channel('ffmpeg')->warning(
            "[SeqFail][OrigReq ID {\$this->originalModelId}] Active stream {\$this->streamType} ID {\$this->activeStreamId} (index {\$this->currentIndexInSourceIds}) failed health check. Initiating sequential failover."
        );
        ...
        for (\$i = \$nextStreamIndexToAttempt; \$i < count(\$this->streamSourceIds); \$i++) {
            ...
            try {
                ...
                if (\$newlyStartedStream) {
                    Log::channel('ffmpeg')->info(
                        "[SeqFail][OrigReq ID {\$this->originalModelId}] Successfully failed over to new stream: {\$this->streamType} ID {\$newlyStartedStream->id} (index {\$i}). Monitoring will continue under its new job."
                    );
                    return;
                } else {
                    Log::channel('ffmpeg')->warning(
