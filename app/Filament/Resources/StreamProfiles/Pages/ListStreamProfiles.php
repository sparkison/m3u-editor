                ...
                ->action(function () {
                    $defaultProfiles = [
                        ...
                        [
                            ...
                            'args' => '-fflags +genpts+discardcorrupt+igndts -i {input_url} -c:v libx264 -preset faster -crf {crf|23} -maxrate {maxrate|2500k} -bufsize {bufsize|5000k} -c:a aac -b:a {audio_bitrate|192k} -hls_time 2 -hls_list_size 30 -hls_flags delete_threshold+program_date_time -hls_delete_threshold 5 -hls_segment_filename segment_%03d.ts -f hls {output_args|index.m3u8}',
                            'format' => 'm3u8',
                        ],
                    ];
